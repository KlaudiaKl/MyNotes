...
@HiltViewModel
class AddEditViewModel @Inject constructor(private val savedStateHandle: SavedStateHandle) :
    ViewModel() {
    ...
    private fun getSelectedNote() {
        if (uiState.selectedNoteId != null) {
            viewModelScope.launch {
                MongoDbRepositoryImpl.getSelectedNote(
    ...
    private suspend fun insertNote(
        ...
        val result = MongoDbRepositoryImpl.insertNote(note = note.apply {})
    ...
    private suspend fun updateNote(
        ...
        val result = MongoDbRepositoryImpl.updateNote(note = note.apply {
    ...
    fun deleteNote(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            if (uiState.selectedNoteId != null) {
                val result = MongoDbRepositoryImpl.deleteNote(id = ObjectId.invoke(uiState.selectedNoteId!!))
                if (result is RequestState.Success) {
                    withContext(Dispatchers.Main) {
